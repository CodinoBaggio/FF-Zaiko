function sheetsToCsv() {
  ...
  const ss = SpreadsheetApp.openById("1muSaplToXU0_60Bm1fye49vRzQbAOcC9fxSog29qNqk")  
  ...
  for(i=0;i<fileIDs.length;i++){
    ...
    const sheet_master = SpreadsheetApp.openById("1muSaplToXU0_60Bm1fye49vRzQbAOcC9fxSog29qNqk").getSheetByName("新旧金立上");

    var masterValues = sheet_master.getRange(2,1,sheet_master.getLastRow()-1, 6).getDisplayValues();
    // console.log(masterValues[0][2]);
        ...
        var lastRow_dateValues = sheet.getRange(2,17).getNextDataCell(SpreadsheetApp.Direction.DOWN).getRow();

        console.log(lastRow_dateValues);

        var dateValues = sheet.getRange(2,17,lastRow_dateValues-1,1).getDisplayValues();
        ...
        for(j=0;j<dateValues.length;j++){
          ...
          values= values.map(elm => [elm[1],elm[2],elm[4],elm[5],elm[6],elm[7],elm[8],elm[9],elm[10],"",elm[14],elm[15],"","",elm[0],""]);
