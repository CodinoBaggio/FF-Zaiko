function convertNewFiles_kinryu_shita(newFiles, param) {
  ...
  const SHEEET_master =
    SpreadsheetApp.getActiveSpreadsheet().getSheetByName('金立下SAマスタ');

  var masterValues = SHEEET_master.getRange(
    2,
    1,
    SHEEET_master.getLastRow() - 1,
    6
  ).getDisplayValues();
  ...
  var afterFolderID = sheet.getRange('H18').getValue();
  ...
  while (files.hasNext()) {
    ...
    for (let i = 0; i < values.length; i++) {
      ...
      var masterRow = masterValues.filter((item) => item[2] == values[i][0]);
  ...
  storeFilesIntoGCS_kinryu_shita(newFiles_converted);
}
