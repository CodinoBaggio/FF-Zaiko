...
// var rsaKey = scriptProp.getProperty("key");
// var saEmail = scriptProp.getProperty("email");
  ...
  self.generateJWT_ = function () {
    ...
    for (var i = 0; i < subAccounts_.length; i++) {
      claim =
        header +
        '.' +
        Utilities.base64Encode(JSON.stringify(makeClaim(subAccounts_[i])));
      JWTs[subAccounts_[i]] = {
        signedClaim:
          claim +
          '.' +
          Utilities.base64Encode(
            Utilities.computeRsaSha256Signature(claim, rsaKey_)
          ),
        expire: expireTime_,
      };
    }
    ...
    for (var user in jwts_) {
      ...
      if (response.error) {
        if (response.error === 'invalid_grant') {
          ...
          throw new Error(
            'There was an error requesting a Token from the OAuth server: ' +
              response.error
          );
        }
      }
